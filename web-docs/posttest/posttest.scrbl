...
solves the problem using a different approach.  At the top of your
file for each problem, include a comment indicating which of your two
...
in each BMI classification category. The names in each category 
do not need to be in any particular order. Use the following 
datatype for the report:

@pydisp{
  data Report:
    | bmi-summary(under, healthy, over, obese)
  end
}
...
Design a program @pyin{data-smooth} that consumes a list of personal 
health records and produces a list of the smoothed @pyin{heart-rate} 
values (not the entire records).


@subsection{Most Frequent Words}

Given a list of strings, design a program @pyin{frequent-words} that 
...
report should be an instance of the following datatype:
@pydisp{
  data Report:
